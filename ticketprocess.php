...
switch (@$task)
{
	case 'ticketadd':
		...
		$srvid= mysql_real_escape_string($_POST['serverID']);
		$msg = mysql_real_escape_string($_POST['message']);
		if (empty($subject)) $error .= 'No subject specified. ';
		if (empty($msg)) $error .= 'No message specified. ';
		if (!empty($error))
		{
		...
		if ($srvid == 'none')
		{
		query_basic( "INSERT INTO `".DBPREFIX."tickets` SET
				`subject` = '".$subject."',
				`creator` = '".$_SESSION['clientid']."',
				`ts_created` = now(),
				`ts_updated` = now(),
				`status` = '1',
				`server` = null");
		...
		header( "Location: viewticket.php?tid=".urlencode($tid));
		die();
	break;
	case 'remove':
	...
	case 'ticketreply':
		...
		if($tid > 0) {
			$ticketqry = mysql_query( "SELECT * from`".DBPREFIX."tickets` WHERE `ticketid` = '".$tid."' and `status`=1 and `creator` = '".$_SESSION['clientid']."'" );
			$ticketdata = mysql_fetch_assoc($ticketqry);
			if(!$ticketdata){
				$error .= 'Ticket not Found';	
			}
